...
public class BasicOperationTest {
  ...
  public void connect() {
    ...
    server = RSocketFactory.receive().acceptor((p, cr) -> Mono.just(requestHandler)).transport(
        LocalServerTransport.create("test-local-server-"
            + testName)).start().block();

    client = RSocketFactory.connect().transport(
        LocalClientTransport.create("test-local-server-" + testName)).start().block();
  }
